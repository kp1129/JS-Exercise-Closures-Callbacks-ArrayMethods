...
function processSum(numberList, callback) {
  return callback(numberList.reduce((acc, currentValue) => {
...
function processContains(item, list, callback) {
  return callback(list.includes(item, 0));
}
...
function getFullNames(runners) {
  ...
  runners.forEach(runner => {
    let name = `${runner.last_name}, ${runner.first_name}`;
...
function firstNamesAllCaps(runners) {
  const runnerNames = [];
  runners.map(runner => runnerNames.push(runner.first_name.toUpperCase()));
  return runnerNames;
}
